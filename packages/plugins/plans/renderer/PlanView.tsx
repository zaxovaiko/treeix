...
import { readPlan, subscribePlans } from './api'
...
export function PlanView({
  ...
  // Agents keep editing the plan after you open it; rereads when the shared plans poll sees it modified (or gone)
  useEffect(() => {
    const load = (): void => {
    ...
    load()
    return subscribePlans((plans) => {
      const modifiedAt = plans.find((candidate) => candidate.name === plan.name)?.modifiedAt ?? -1
      if (loadedAt !== null && modifiedAt !== loadedAt) load()
      loadedAt = modifiedAt
    })
  }, [plan.name])
