...
- **Contributions, not hooks.** A renderer plugin returns data: `tabs`, `panels`, `titleBar`, `Root`, `Settings`, `commands`, `codeBlocks`, `linkPreviews`, `toolMarks`, `services`. The host sorts and renders them. There is no plugin that patches the host.
